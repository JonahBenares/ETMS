...
class Report extends CI_Controller {
    ...
    public function history_view(){  
        ...
        if($row_return!=0){
            foreach($this->super_model->custom_query("SELECT rh.accountability_id, rh.return_date, rh.received_by,rd.et_id FROM return_head rh INNER JOIN return_details rd ON rh.return_id = rd.return_id WHERE rd.ed_id = '$id' GROUP BY rd.return_id") AS $ret){
    ...
    /*public function search_report(){
        ...
        if(!empty($_POST['category'])){
            $sql.=" et_head.category_id = '$_POST[category]' AND";
            $cat = $this->super_model->select_column_where("category", "category_name", "category_id", $_POST['category']);
            $filter .= "Category - ".$cat.", ";
        }

        if(!empty($_POST['subcat'])){
            $sql.=" et_head.subcat_id = '$_POST[subcat]' AND";
            $subcat = $this->super_model->select_column_where("subcategory", "subcat_name", "subcat_id", $_POST['subcat']);
            $filter .= "Sub Category - ".$subcat.", ";
        }

        if(!empty($_POST['department'])){
            $sql.=" et_head.department LIKE '%$_POST[department]%' AND";
        ...
        if(!empty($_POST['item'])){
            $sql.=" et_head.et_desc LIKE '%$_POST[item]%' AND";

            $filter .= "Item Desc - ".$_POST['item'].", ";
        }

        if(!empty($_POST['brand'])){
            $sql.=" et_details.brand LIKE '%$_POST[brand]%' AND";
            $filter .= "Brand - ".$_POST['brand'].", ";
        }

        if(!empty($_POST['model'])){
            $sql.=" et_details.model LIKE '%$_POST[model]%' AND";
            $filter .= "Model - ".$_POST['model'].", ";
        }

        if(!empty($_POST['item_type'])){
            $sql.=" et_details.type LIKE '%$_POST[item_type]%' AND";
            $filter .= "Type - ".$_POST['item_type'].", ";
        }

        if(!empty($_POST['serial_no'])){
            $sql.=" et_details.serial_no LIKE '%$_POST[serial_no]%' AND";
            $filter .= "Serial No. - ".$_POST['serial_no'].", ";
        }

        if(!empty($_POST['damage'])){
            $sql.=" et_details.damage = '$_POST[damage]' AND";
            $filter .= "Damage Items, ";
        }
        ...
        foreach($this->super_model->select_row_where('et_head', 'et_id', $et_id) AS $ret){
            if($checked < $ret->qty){
                ...
                if($this->super_model->update_where('et_head', $data, 'et_id', $et_id)){
                    ...
                    echo "<script>window.location = '".base_url()."index.php/report/tag_damage_form/$id/$et_id';</script>";
                }
            }
        } 
        $y++;
    ...
    public function tag_damage_form(){  
        ...
        $data['checked_id'] = $this->super_model->select_column_where("employees", "employee_id", "employee_name", 'Mary Grace Bugna'); 
    ...
    public function damage_report(){  
        ...
        foreach($this->super_model->select_row_where('damage_info', 'ed_id', $ed_id) AS $dam){
    ...
    public function assignlist(){
     ...
     public function itemlist(){
        ...
        if($rows!=0){
            ...
            ?>
                   <li onClick="selectItem('<?php echo $itm->et_id; ?>','<?php echo $det->ed_id; ?>','<?php echo $itm->et_desc; ?>','<?php echo $det->asset_control_no;?>','<?php echo $det->acquisition_date; ?>','<?php echo $det->serial_no; ?>','<?php echo $det->brand; ?>','<?php echo $det->model; ?>','<?php echo $qty; ?>','<?php echo $unit; ?>','<?php echo $det->unit_price; ?>','<?php echo $total; ?>',)"><?php echo $itm->et_desc." - ".$det->brand." - ".$det->serial_no." - ".$det->model; ?></li>
            <?php 
                }
            }
            echo "<ul>";
    ...
    public function export_equipment(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K2', "Brand");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M2', "Model");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O2', "Serial No.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q2', "QTY");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R2', "UOM");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S2', "Date Issued ");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U2', "Accountability");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('W2', "Office / Department");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Y2', "Unit Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z2', "Total Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AA2', "Remarks");
        $styleArray = array(
          'borders' => array(
            'allborders' => array(
        ...
        foreach ($this->super_model->select_join_where("et_head", "et_details", $query, "et_id") AS $et){
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$num, $asset_control_no);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$num, $acquisition_date);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$num, $et->et_desc);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$num, $brand);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$num, $model);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$num, $serial_no);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$num, $et->qty);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R'.$num, $unit);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$num, $date_issued);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U'.$num, $employee);
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AA'.$num, $remarks);
