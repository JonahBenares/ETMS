...
class Borrow extends CI_Controller {
    ...
    public function insert_borrow_head(){
        ...
        if($this->super_model->insert_into("borrow_head", $data)){
            echo "<script>window.location ='".base_url()."index.php/borrow/borrow_add/$bh_id'; </script>";
        }
    }
    ...
    public function borrow_view(){  
        ...
        if($count!=0){
            foreach($this->super_model->custom_query("SELECT * FROM borrow_head WHERE ".$query) AS $head){
                ...
                /*$data['employee'][] = array(
                    'employee'=>$employee,
                    'series'=>$head->borrow_series,
                    'date'=>$head->borrowed_date,
                );*/
                /*select_row_where("borrow_details","bh_id",$head->bh_id)*/
    ...
    public function generateReturn(){
       ...
       ?>
       <script>
        window.location.href ='<?php echo base_url(); ?>index.php/borrow/borrow_view/<?php echo $return_id; ?>'</script> <?php
    }
    ...
    public function returnlist(){
        ...
        if($rows!=0){
            ...
            foreach($this->super_model->select_custom_where("employees", "employee_name LIKE '%$return%'") AS $itm){
               ...
               /* $test =$this->super_model->count_rows_where('borrow_details', "returned",'0');*/
               /* $returned = $this->super_model->select_column_where("borrow_details", "returned", "bh_id", $he->bh_id);*/
                /*foreach($this->super_model->select_row_where("borrow_details", "returned",'0') AS $det){*/
                /*foreach($this->super_model->select_custom_where("borrow_head", "borrowed_by = '$itm->employee_id'") AS $he){*/ 
                /*
                    foreach($this->super_model->select_custom_where("borrow_details", "bh_id = '$he->bh_id'") AS $det){
                        foreach($this->super_model->select_custom_where("et_head", "et_id = '$det->et_id'") AS $un){
                            $unit = $this->super_model->select_column_where("unit", "unit_name", "unit_id", $un->unit_id);
                        }
                        $et_id = $this->super_model->select_column_where("et_head", "et_id", "et_id", $det->et_id);
                        $ed_id = $this->super_model->select_column_where("et_details", "ed_id", "ed_id", $det->ed_id);
            ...
            if(isset($e_id[$z])){
                ...
                foreach($this->super_model->select_row_where('et_head', 'et_id', $et_id[$z]) AS $ret){
                    /*if($checked < $ret->qty){
                        ...
                        if($this->super_model->insert_into("et_head", $et_data)){
                            //for($x=0;$x<$checked;$x++){
                                //$date_issued = $this->super_model->select_column_where("et_details", "date_issued", "ed_id", $e_id[$z]);
                            ...
                            $this->super_model->update_where('et_head', $qty_data, 'et_id', $et_id[$z]);
                            echo "<script>window.location = '".base_url()."index.php/report/tag_damage_form/$returned_by/$new_etid';</script>";
                        }
                        ...
                        if($this->super_model->update_where('et_head', $data, 'et_id', $et_id[$z])){
                            ...
                            echo "<script>window.location = '".base_url()."index.php/borrow/tag_damage_form/$returned_by'; </script>";
                        }
                    //}
                } 
                //$y++;
    ...
    public function insert_damage_form(){
        ...
        foreach($this->super_model->select_row_where("borrow_details","ed_id",$edid) AS $up){
            $this->super_model->update_where('borrow_details', $bdet_data, 'bd_id', $up->bd_id);
        }*/
        echo "<script>alert('Successfully Tagged as Damage!'); window.location = '".base_url()."index.php/borrow/tag_damage_print';</script>";
    }
            ...
            if(!empty($e_id[$x])){
                ...
                foreach($this->super_model->select_row_where('et_head', 'et_id', $et_id) AS $ret){
                    if($checked < $ret->qty){
                        ...
                        if($this->super_model->insert_into("et_head", $et_data)){
                            for($x=0;$x<$checked;$x++){
                                $date_issued = $this->super_model->select_column_where("et_details", "date_issued", "ed_id", $e_id[$x]);
                                foreach($this->super_model->select_row_where('et_details', 'ed_id', $e_id[$x]) AS $det){
                                    $det_data = array(
                                        'et_id'=>$new_etid,
                                        'date_issued'=>'',
                                        'damage'=>1,
                                    ...
                                    $this->super_model->update_where("et_details", $det_data, "ed_id", $e_id[$x]);
                                }
                            }
                            $bdet_data = array(
                                'et_id'=>$new_etid,
                        ...
                        if($this->super_model->update_where('et_head', $data, 'et_id', $et_id)){
                            for($x=0;$x<$checked;$x++){
                                $date_issued = $this->super_model->select_column_where("et_details", "date_issued", "ed_id", $e_id[$x]);
                                $count_exist = $this->super_model->count_custom_where("et_details","et_id = '$ret->et_id' AND ed_id= '$e_id[$x]'");
                                if($count_exist!=0){
                                    foreach($this->super_model->select_row_where('et_details', 'ed_id', $e_id[$x]) AS $det){
                                        $det_data = array(
                                            'date_issued'=>'',
                            ...
                            echo "<script>window.location = '".base_url()."index.php/report/tag_damage_form/$returned_by/$et_id'; </script>";
                        }
                    }
                } 
                $y++;
